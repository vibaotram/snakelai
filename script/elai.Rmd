...
parse_jobname <- function(pos_file, logname) {
  ...
  jobname <- paste(logname, batch, sep = "_")
  return(jobname)
}
...
submit_elai <- lapply(1:nrow(all_params), function(i) {
  job_name <- parse_jobname(as.character(all_params$pos[i]), logname)
  slurm_apply(run_elai, all_params[i,], submit = TRUE,
              jobname = job_name, sh_template = "script/submit_elai_sh.txt",
              slurm_options = list("output" = logfile, "error" = logfile, "mem" = paste0(mem,"G"), "partition" = "long"))
})
```
